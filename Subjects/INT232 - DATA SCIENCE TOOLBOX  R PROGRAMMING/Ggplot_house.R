...
ggplot(data=house, aes(x=factor(rooms), y=price, 
                      fill=factor(rooms))+geom_boxplot()-> o1

o1 +labs(title = "Price/Rooms", x="Rooms",fill="Rooms")->o2

o2+theme(panel.background = element_rect( fill = "palegreen"))->o3

o3+theme(plot.title = element_text(hjust = 0.5, face= "bold",
                                   colour = "blue")) -> o4

o4+scale_y_continuous(labels=dollar)

#theme for facet data
ggplot(data=house, aes(y=price, x=living_area, 
                      col=air_cond))+geom_point()+
  geom_smooth(method="lm",se=F)+facet_grid(~air_cond)->ob1

ob1+theme(panel.background = element_rect( fill = "lemonchiffon2"))-